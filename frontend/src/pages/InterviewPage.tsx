...
export default function InterviewPage() {
  ...
  const submit = useCallback(async (timedOut = false) => {
    ...
    if (!timedOut && answer.trim().length < 10) {
      setError("Please provide a more detailed answer before submitting.");
      return;
    }
    ...
    try {
      ...
      setTimeline((prev) => [
        ...prev,
        {
          ...
          score: typeof res.data.answer?.score === "number"
            ? Math.round(res.data.answer.score)
            : undefined
        }
      ]);
  ...
  const start = async () => {
    try {
      ...
      await api.post(`/sessions/${created.data.id}/analyze`, {
        resumeText,
        jdText
      });
  ...
  const progress = Math.min(
    100,
    (questionNumber / TOTAL_QUESTIONS_GUIDE) * 100
  );

  const stageLabel = question
    ? stageByRound[question.round] || "Interview Round"
    : "Interview Summary";
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 max-w-6xl mx-auto relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-wrap justify-between items-start gap-3 mb-6">
        <div>
          <p className="text-sm uppercase tracking-[0.18em] text-[#46a6ff]">
            HireMind AI
          </p>

          <h1 className="text-3xl md:text-5xl font-semibold leading-tight">
            Adaptive Interview Intelligence Platform
          </h1>

          <p className="text-slate-300 mt-2">
            Guided, structured interview simulation with live adaptation and readiness scoring.
          </p>
      ...
      <div className="relative z-10 glass rounded-2xl p-4 mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm text-slate-200">
          <span className="ai-dot" /> AI Interviewer Online
        </div>

        <p className="text-sm text-slate-300">
          Step {step} of 2 {step === 1 ? "-> Candidate Analysis" : "-> Adaptive Interview"}
        </p>
      </div>

      {error && (
        <div className="glass border border-red-500/40 rounded-xl p-3 mb-4 text-red-200 relative z-10">
          {error}
        </div>
      )}
      ...
      {!session && (
        <motion.div
          ...
          className="glass p-5 md:p-6 rounded-2xl space-y-4 relative z-10"
        >
          <label className="text-sm text-slate-300 flex items-center gap-2">
            <BriefcaseBusiness size={16} /> Target Role
          </label>
          ...
          <label className="text-sm text-slate-300 flex items-center gap-2">
            <FileText size={16} /> Resume Context
          </label>
          ...
          <div className="glass rounded-xl p-3">
            ...
            {pdfLoading && (
              <p className="text-xs text-[#7ef7dc] mt-2">
                Parsing PDF and extracting text...
              </p>
            )}

            {pdfError && (
              <p className="text-xs text-red-300 mt-2">{pdfError}</p>
            )}
          </div>
          ...
          <textarea
            ...
            placeholder="Paste resume content for AI analysis"
          />

          <label className="text-sm text-slate-300 flex items-center gap-2">
            <ClipboardList size={16} /> Target Job Description
          </label>
          ...
          <textarea
            ...
            placeholder="Paste JD text to calibrate interview"
          />
        ...
        <div className="relative z-10 grid lg:grid-cols-[1fr_300px] gap-4">
          <div className="space-y-4">
            ...
            <div className="glass rounded-2xl p-4 md:p-5">
              <div className="flex flex-wrap justify-between items-center gap-2 text-sm">
                <span className="text-slate-200">
                  Question {questionNumber} of {TOTAL_QUESTIONS_GUIDE}
                </span>

                <div className="flex items-center gap-2">
                  <span className="px-2 py-1 rounded-full bg-[#0d1f38] text-[#7fc7ff]">
                    {question.category}
                  </span>

                  <span className="px-2 py-1 rounded-full bg-[#10291f] text-[#7ef7dc]">
                    {labelByDifficulty[session.currentDifficulty]}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 grid sm:grid-cols-3 gap-2 text-sm">
                <div className="glass rounded-xl px-3 py-2 flex items-center gap-2">
                  <Target size={14} className="text-[#46a6ff]" />
                  Stage: {stageLabel}
                </div>

                <div className="glass rounded-xl px-3 py-2 flex items-center gap-2">
                  <Gauge size={14} className="text-[#2df7c4]" />
                  Live Readiness: {readiness.toFixed(1)}
                </div>

                <div className="glass rounded-xl px-3 py-2 flex items-center gap-2">
                  <Brain size={14} className="text-[#b7c9ff]" />
                  AI Adaptation: Active
                </div>
              </div>

              <p className="mt-5 text-lg md:text-2xl leading-relaxed">
                {question.prompt}
              </p>
              ...
              <div className="mt-4">
                <div className="h-2 rounded-full bg-slate-800 overflow-hidden">
                  <div
                    className={`h-full ${danger ? "bg-red-400" : "bg-[#2df7c4]"}`}
                    style={{
                      width: `${timerPercent}%`,
                ...
                <div className="mt-2 flex justify-between items-center">
                  <p className={`text-sm ${danger ? "text-red-300" : "text-slate-300"}`}>
                    {left}s remaining
                  </p>

                  <p className="text-xs text-slate-400">
                    {Math.max(0, TOTAL_QUESTIONS_GUIDE - questionNumber)} questions remaining
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="glass rounded-2xl p-4 border border-white/5">
              <p className="text-xs uppercase tracking-[0.16em] text-[#46a6ff]">
                Recruiter Evaluation Criteria
              </p>

              <div className="mt-3 flex flex-wrap gap-2 text-xs">
                <span className="glass rounded-full px-3 py-1">Technical Depth</span>
                <span className="glass rounded-full px-3 py-1">Communication Clarity</span>
                <span className="glass rounded-full px-3 py-1">Optimization Thinking</span>
                <span className="glass rounded-full px-3 py-1">Contextual Relevance</span>
              </div>
            </div>
            ...
            <textarea
              ...
              placeholder="Respond with structured technical clarity..."
            />
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                {submitLoading ? (
                  <>
                    <Loader2 size={16} className="animate-spin" /> AI Evaluating...
                  </>
                ) : (
                  <>
              ...
              <button
                disabled={submitLoading}
                onClick={() => submit(true)}
                className="px-5 py-2 border border-slate-600 rounded-lg"
              >
                Force Timeout Submit
              </button>
            </div>
          </div>
          ...
          <aside className="glass rounded-2xl p-4 h-fit">
            ...
            <div className="space-y-2 max-h-[420px] overflow-auto pr-1">
              ...
              {timeline.map((item, idx) => (
                <div
                  key={`${item.q}-${idx}`}
                  className="rounded-xl bg-[#0c1427] px-3 py-3 border border-white/5"
                >
