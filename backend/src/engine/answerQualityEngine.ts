...
const tokens = (text: string) => normalize(text).split(" ").filter((t) => t.length > 2);

const tokenSet = (text: string) => new Set(tokens(text));
...
const overlapRatio = (shortText: string, longText: string) => {
...
const fluffScore = (answer: string) => {
  const n = normalize(answer);
  let score = 0;
  for (const p of fluffPhrases) {
    if (n.includes(p)) score += 18;
  }
...
const intentValidity = (question: string, answer: string) => {
  ...
  if (qIsBehavioral) {
    if (/(i|we)\s/.test(a) && /because|result|learned|improved|handled/.test(a)) return 90;
    return 35;
  }
  if (qIsTechnical) {
    if (/because|tradeoff|complexity|cache|latency|state|render|query|index|scal/.test(a)) return 88;
    return 40;
  }
  return 65;
};

const isDuplicateAnswer = (answer: string, previousAnswers: string[]) => {
  for (const prev of previousAnswers) {
    if (jaccard(answer, prev) > 0.82) return true;
  }
  return false;
};

export const evaluateQualitySignals = (input: QualityInput): QualitySignals => {
  ...
  if (qRel < 28) flags.push("Low question relevance");
  if (dup) flags.push("Duplicate/repeated answer");
  if (fluff > 45) flags.push("Generic filler/fluff");
  if (resumeSim > 45 || hasLikelyResumePattern(input.answer)) flags.push("Looks like pasted resume content");
  if (jdSim > 45 || hasLikelyJdPattern(input.answer)) flags.push("Looks like pasted JD content");
  if (intent < 45) flags.push("Weak intent alignment");
...
export const applyQualityPenalties = (metrics: EvalMetrics, signals: QualitySignals) => {
